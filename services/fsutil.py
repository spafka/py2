...
def lock(lock_name: Optional[str] = None, timeout: int = 10):
    def outer(func):
        print(lock_name)
        ...
        def inner(*agrs, **kwargs):
            ...
            lock_key = lock_name;
            if lock_key is None:
                if "lock_name" in kwargs:
                    lock_key = kwargs["lock_name"]
                else:
                    raise KeyError("lock_name not set")
            while True:
                try:
                    conn.execute("INSERT INTO distributed_locks (lock_name,cts) VALUES (:lock,:time)",
                                 {"lock": lock_key, "time": datetime.datetime.now()})
                    conn.commit()
                    try:
...
if __name__ == '__main__':
    ...
    print(find_files("./", ".py",lock_name="xxx"))
